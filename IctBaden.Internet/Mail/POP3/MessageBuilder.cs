...
namespace IctBaden.Internet.Mail.POP3
{
	...
	internal static class MessageBuilder
    {
		...
		private static NameValueCollection ParseMailHeader(string header)
        {
		...
		private static MIMEPart[] ParseMailBody(string body, NameValueCollection header)
        {
		...
		private static Bodypart BodypartFromMIME(MIMEPart mimePart)
        {
		    ...
		    if (mimePart.header["Content-Disposition"] == null) return bodypart;
