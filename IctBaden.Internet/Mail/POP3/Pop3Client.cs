...
namespace IctBaden.Internet.Mail.POP3
{
    ...
    /// 
    /// From Torben Könke's work.
    /// http://smiley22.github.com/S22.Pop3/Documentation
    /// </summary>
...
#pragma warning restore 1589
        ...
        {
            if(TraceEnabled) Trace.TraceInformation("POP3: Send: " + command);
                    ...
                    while (true)
                    {
                        ...
                        switch (b)
                        {
                            case carriageReturn:
                                continue;
                            case newline:
                                var response = Encoding.ASCII.GetString(mem.ToArray());
                                if(TraceEnabled) Trace.TraceInformation("POP3: Response: " + response);
                                return response;
                            default:
                                mem.WriteByte(b);
                                break;
        ...
        public void Login(string username, string password, AuthMethod method)
        {
            ...
            switch (method)
            {
                ...
                case AuthMethod.CRAMMD5:
                    ...
                    // retrieve server key
                    var key = Encoding.Default.GetString(Convert.FromBase64String(response.Replace("+ ", "")));
                    // compute the hash
                    using (var kMd5 = new HMACMD5(Encoding.ASCII.GetBytes(password)))
                    {
                        ...
                        var command = Convert.ToBase64String(Encoding.ASCII.GetBytes(username + " " + key));
...
#pragma warning restore 1589
        public string[] Capabilities()
        {
            ...
            lock (sequenceLock)
            {
                var response = SendCommandGetResponse("CAPA");
...
#pragma warning restore 1589
        public MessageInfo[] GetStatus()
        {
            ...
            lock (sequenceLock)
            {
                var response = SendCommandGetResponse("LIST");
                if (!IsResponseOK(response))
                    throw new BadServerResponseException(response);
                while ((response = GetResponse()) != ".")
                {
                    var m = Regex.Match(response, @"(\d+)\s(\d+)");
                    if (!m.Success)
                        continue;
                    var number = Convert.ToUInt32(m.Groups[1].Value);
                    var size = Convert.ToUInt64(m.Groups[2].Value);
...
#pragma warning restore 1589
        public MailMessage GetMessage(uint number, FetchOptions options = FetchOptions.Normal, bool delete = false)
        {
            if (!Authed)
                throw new NotAuthenticatedException();
            var command = options == FetchOptions.HeadersOnly ? ("TOP " + number + " 0") : ("RETR " + number);
            ...
            return options == FetchOptions.HeadersOnly ? MessageBuilder.FromHeader(builder.ToString()) : MessageBuilder.FromMIME822(builder.ToString());
        }
...
#pragma warning restore 1589
        public MailMessage[] GetMessages(uint[] numbers = null, FetchOptions options = FetchOptions.Normal, bool delete = false)
        {
            if (numbers == null)
                numbers = GetMessageNumbers();
            return numbers.Select(n => GetMessage(n, options, delete)).ToArray();
        }
        ...
        public void DeleteMessage(uint number)
        {
            ...
            lock (sequenceLock)
            {
                var response = SendCommandGetResponse("DELE " + number);
                if (!IsResponseOK(response))
                    throw new BadServerResponseException(response);
            }
        }
